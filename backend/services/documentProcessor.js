...
class DocumentProcessor {
  ...
  async processFile(filePath, fileType, originalName) {
    try {
      console.log(`📄 Processing ${fileType.toUpperCase()} file: ${originalName}`);
      ...
      const chunks = qaChunks.length > 0 ? qaChunks : this.createChunks(content);
      
      console.log(`✅ Processed ${originalName}: ${content.length} characters, ${chunks.length} chunks (${qaChunks.length > 0 ? 'Q&A format' : 'regular format'})`);
  ...
  async processPDF(filePath) {
    try {
      ...
      if (fileSizeMB > 10) {
        throw new Error('PDF file too large (>10MB). Please use a smaller file.');
      }
      ...
      try {
        ...
        if (text.length > 8000) {
          console.warn(`⚠️  PDF text truncated from ${text.length} to 8000 characters for memory efficiency`);
        ...
        const alternativeText = await alternativePdfProcessor.processPDF(filePath);
      ...
      throw new Error(`Failed to process PDF: ${error.message}. The PDF might be corrupted, password-protected, or contain only images.`);
    }
  }
  ...
  extractQAPairs(content) {
    ...
    for (const pattern of this.qaPatterns) {
      ...
      if (matches.length > 0) {
        console.log(`✅ Found ${matches.length} Q&A pairs using pattern ${this.qaPatterns.indexOf(pattern) + 1}`);
        ...
        for (const match of matches) {
          ...
          if (question && answer && question.length > 5 && answer.length > 5) {
            ...
            const truncatedQuestion = question.length > 500 ? question.substring(0, 500) + '...' : question;
            ...
            console.log(`❌ Skipped match ${pairIndex + 1} - insufficient content`);
          }
        }
        
        // If we found Q&A pairs with this pattern, use them
        if (qaPairs.length > 0) {
          console.log(`🎯 Using ${qaPairs.length} Q&A pairs from pattern ${this.qaPatterns.indexOf(pattern) + 1}`);
          break;
        }
      }
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (this.looksLikeQuestion(line)) {
        ...
        if (currentQuestion && currentAnswer) {
          ...
          const truncatedQuestion = currentQuestion.length > 500 ? currentQuestion.substring(0, 500) + '...' : currentQuestion;
          const truncatedAnswer = currentAnswer.length > 500 ? currentAnswer.substring(0, 500) + '...' : currentAnswer;
  ...
  looksLikeQuestion(text) {
    ...
    return questionIndicators.some(indicator => indicator) && text.length > 10;
  }
  ...
  createChunks(content) {
    ...
    while (startIndex < contentLength && chunkIndex < maxChunks) {
      ...
      if (endIndex < contentLength) {
        ...
        const bestEnd = Math.max(sentenceEnd, questionEnd, exclamationEnd, newlineEnd);
      ...
      if (chunkText.length > 20) { // Only include meaningful chunks
        chunks.push({
          id: uuidv4(),
          text: chunkText,
          metadata: {

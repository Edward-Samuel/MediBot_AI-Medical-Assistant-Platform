...
class EmbeddingService {
  ...
  async initialize() {
    try {
      ...
      console.error(
        "💡 Make sure you have installed @xenova/transformers: npm install @xenova/transformers",
      );
      return false;
    }
  }
      ...
      if (embedding.length !== this.dimension) {
        ...
        if (embedding.length < this.dimension) {
          embedding = embedding.concat(
            new Array(this.dimension - embedding.length).fill(0),
          );
        } else {
  ...
  async generateBatchEmbeddings(texts) {
    ...
    try {
      ...
      console.log(
        `Processing ${texts.length} embeddings with aggressive memory optimization...`,
      );
  ...
  getModelInfo() {
    return {
      ...
      description:
        "all-MiniLM-L6-v2 - Lightweight general-purpose embedding model optimized for memory efficiency",
    };
  }
}

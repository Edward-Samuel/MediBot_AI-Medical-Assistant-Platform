...
class PineconeService {
  ...
  async initialize() {
    try {
      if (!process.env.PINECONE_API_KEY) {
        console.log('⚠️  Pinecone API key not found - FAQ system will be disabled');
        return false;
      }
      ...
      if (!embeddingInitialized) {
        console.log('⚠️  Embedding service failed to initialize - FAQ embeddings will be disabled');
        return false;
      }
  ...
  async upsertChunks(chunks, faqId, metadata = {}) {
    ...
    try {
      ...
      const qaPairs = chunks.filter(chunk => chunk.metadata.type === 'qa_pair' || chunk.metadata.type === 'detected_qa');
      const regularChunks = chunks.filter(chunk => chunk.metadata.type === 'content_chunk');
      
      console.log(`📊 Found ${qaPairs.length} Q&A pairs and ${regularChunks.length} regular chunks`);
      ...
      for (let i = 0; i < qaPairs.length; i++) {
        ...
        try {
          ...
          console.error(`❌ Error processing Q&A pair ${i + 1}:`, error.message);
          continue;
        }
      }
      ...
      for (let i = 0; i < maxRegularChunks; i++) {
        ...
        console.log(`   Processing content chunk ${i + 1}/${maxRegularChunks}...`);
        ...
        try {
          ...
          console.log(`   ✅ Processed content chunk ${i + 1}/${maxRegularChunks}`);
          ...
          console.error(`❌ Error processing content chunk ${i + 1}:`, error.message);
          continue;
        }
      }

      if (regularChunks.length > maxRegularChunks) {
        console.warn(`⚠️  Processed only ${maxRegularChunks} out of ${regularChunks.length} content chunks for memory efficiency`);
      }

      console.log(`✅ Completed processing ${processedIds.length} chunks for FAQ ${faqId} (${qaPairs.length} Q&A pairs, ${Math.min(regularChunks.length, maxRegularChunks)} content chunks)`);
      return processedIds;

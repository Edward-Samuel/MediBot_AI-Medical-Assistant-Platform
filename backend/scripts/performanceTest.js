...
class PerformanceTest {
  ...
  async runComparisonTest() {
    ...
    for (const testData of testMessages) {
      await this.testEndpoint('/api/ai/chat', {
    ...
    for (const testData of testMessages) {
      await this.testEndpoint('/api/ai/chat-optimized', {
        message: testData.message,
        language: 'en',
        conversationHistory: [],
        forceWebSearch: testData.forceWebSearch || false
  ...
  generateReport() {
    ...
    const originalResults = this.results.filter(r => r.endpoint.includes('/chat') && !r.endpoint.includes('optimized'));
    const optimizedResults = this.results.filter(r => r.endpoint.includes('optimized'));

    if (originalResults.length > 0 && optimizedResults.length > 0) {
      const originalAvg = originalResults.reduce((sum, r) => sum + r.avgTime, 0) / originalResults.length;
      const optimizedAvg = optimizedResults.reduce((sum, r) => sum + r.avgTime, 0) / optimizedResults.length;
      ...
      if (improvement > 0) {
        ...
        console.log(`   ⚠️  Optimization needs work. ${Math.abs(improvement).toFixed(1)}% slower`);
      }
    }

...
class AdminChatHistoryTester {
   ...
   */
  async loginAsAdmin(credentials = { email: 'admin@example.com', password: 'admin123' }) {
    try {
  ...
  async testGetAllChatHistory() {
    try {
      ...
      if (response.data.sessions.length > 0) {
        ...
        console.log(`👤 User: ${firstSession.user?.name || 'Unknown'} (${firstSession.user?.email || 'No email'})`);
      ...
      console.error('❌ Failed to get chat history:', error.response?.data?.message || error.message);
      return null;
    }
  }
  ...
  async testGetChatSessionDetails(sessionId) {
    try {
      ...
      const response = await axios.get(`${this.baseUrl}/admin/chat-history/${sessionId}`, {
        headers: { Authorization: `Bearer ${this.adminToken}` }
      });
      ...
      if (response.data.messages.length > 0) {
        console.log(`📄 First message: ${response.data.messages[0].content.substring(0, 50)}...`);
        console.log(`📄 Last message: ${response.data.messages[response.data.messages.length - 1].content.substring(0, 50)}...`);
      }

      return response.data;

    } catch (error) {
      console.error('❌ Failed to get session details:', error.response?.data?.message || error.message);
      return null;
    }
  }
  ...
  async testGetChatHistoryStats() {
    try {
      ...
      const response = await axios.get(`${this.baseUrl}/admin/chat-history/stats/overview`, {
        headers: { Authorization: `Bearer ${this.adminToken}` }
      });
      ...
      if (response.data.languageBreakdown.length > 0) {
        ...
        response.data.languageBreakdown.forEach(lang => {
          console.log(`  ${lang._id}: ${lang.count} sessions, ${lang.totalMessages} messages`);
        });
      }
      ...
      console.error('❌ Failed to get chat statistics:', error.response?.data?.message || error.message);
      return null;
    }
  }
  ...
  async testSearchChatHistory(searchQuery = 'diabetes') {
    try {
      ...
      console.log(`📋 Found ${response.data.sessions.length} sessions matching "${searchQuery}"`);
      
      response.data.sessions.forEach((session, index) => {
        console.log(`  ${index + 1}. ${session.sessionId} - ${session.title}`);
        console.log(`     User: ${session.user?.name || 'Unknown'} | Messages: ${session.messageCount}`);
      ...
      console.error('❌ Failed to search chat history:', error.response?.data?.message || error.message);
      return null;
    }
  }
  ...
  async testFilterByUser(userId) {
    try {
      ...
      console.log(`📋 Found ${response.data.sessions.length} sessions for user ${userId}`);
      
      return response.data;

    } catch (error) {
      console.error('❌ Failed to filter by user:', error.response?.data?.message || error.message);
      return null;
    }
  }
  ...
  async runTests() {
    ...
    if (allHistory && allHistory.sessions.length === 0) {
      ...
      console.log('   Create some chat sessions by using the chat functionality first.');
    }
  }
}

...
const testCases = [
  ...
  },
  {
    ...
    expectedIntent: "general_chat",
    description: "Personal medical concern should be general chat (no emergency detection)"
  },
  {
    message: "I think I'm having a heart attack",
    expectedIntent: "general_chat",
    description: "Personal medical emergency should be general chat (no emergency detection)"
  },
  {
    message: "Help! I can't breathe",
    expectedIntent: "general_chat",
    description: "Urgent personal concern should be general chat (no emergency detection)"
  }
];

async function testIntentClassification() {
  console.log('🔍 Testing Intent Classification (Emergency Detection Removed)\n');
  ...
  for (const testCase of testCases) {
    try {
      ...
      const result = await intentClassifier.classifyIntent(testCase.message, []);
      
      console.log(`🤖 Classified as: ${result.intent} (confidence: ${result.confidence.toFixed(2)})`);
      ...
      if (isCorrect) {
        ...
        console.log(`❌ Expected ${testCase.expectedIntent} but got ${result.intent}`);
      }
  ...
  console.log('='.repeat(80));
  console.log(`✅ Correct classifications: ${correctClassifications}/${totalTests}`);
  ...
  // Without force
  const normalResult = await intentClassifier.classifyIntent(medicalMessage, []);
  console.log(`Normal classification: ${normalResult.intent}`);
  
  // With force
  const forcedResult = await intentClassifier.classifyIntent(medicalMessage, [], { forceWebSearch: true });
...
async function testSearchPatterns() {
  ...
  for (const pattern of searchPatterns.slice(0, 5)) { // Test first 5 patterns
    for (const term of medicalTerms.slice(0, 3)) { // Test first 3 terms
      const message = `${pattern} ${term}`;
      const result = await intentClassifier.classifyIntent(message, []);
      
      console.log(`"${message}" → ${result.intent} (${result.confidence.toFixed(2)})`);
    }
  }
}

...
async function continueConversationExample() {
  try {
    ...
    console.log('Full conversation length:', continuedResponse.fullConversation.length);
...
async function medicalAnalysisExample() {
  try {
    ...
    const analysis = await openRouterService.analyzeSymptoms(symptoms, patientInfo);
...
if (require.main === module) {
  async function runExamples() {
    ...
    if (!process.env.OPENROUTER_API_KEY) {
      console.log('❌ OPENROUTER_API_KEY not configured in environment variables');

...
function testEmergencyDetection() {
  ...
  emergencyTests.forEach(test => {
    const isEmergency = responseTemplates.isEmergency(test.message, test.language);
    const status = isEmergency === test.expected ? '✅ PASS' : '❌ FAIL';
    console.log(`${status} Emergency Detection: "${test.message}" (${test.language}) -> ${isEmergency}`);
  });
...
function testSymptomExtraction() {
  ...
  symptomTests.forEach(test => {
    const symptom = responseTemplates.extractSymptom(test.message, test.language);
    const status = symptom.includes(test.expected) ? '✅ PASS' : '❌ FAIL';
    console.log(`${status} Symptom Extraction: "${test.message}" -> "${symptom}"`);
  });
...
function testResponseGeneration() {
  ...
  testCases.forEach(testCase => {
    ...
    if (responseTemplates.isEmergency(testCase.message, testCase.language)) {
      ...
      const symptom = responseTemplates.extractSymptom(testCase.message, testCase.language);
      
      if (symptom !== (testCase.language === 'ta' ? 'உங்கள் அறிகுறி' : 'your symptom')) {
        response = responseTemplates.generateMedicalResponse(testCase.message, symptom, testCase.language);
        console.log('Type: Medical Response');
      } else {
        response = responseTemplates.generateGeneralResponse(testCase.message, testCase.language);
...
function validateResponse(response, language) {
  ...
  results['Appropriate Length'] = response.length > 50 && response.length < 1000;
  
  // Check for structured formatting
  results['Structured Format'] = response.includes('**') || response.includes('•');

...
async function testIntentClassifier() {
  ...
  for (const test of testMessages) {
    try {
      ...
      console.log(`🤖 Predicted: ${result.intent} (confidence: ${result.confidence.toFixed(2)})`);
      console.log(`🔍 Method: ${result.method}`);
      console.log(`💭 Reasoning: ${result.reasoning}`);
      
      if (result.scores) {
        console.log(`📊 Scores: FAQ=${result.scores.faq.toFixed(2)}, General=${result.scores.general_chat.toFixed(2)}`);
      }
...
async function testWithHistory() {
  ...
  const conversationHistory = [
    ...
    { role: 'bot', content: 'Hello! I can help you with medical questions and appointment booking.' },
    { role: 'user', content: 'I have been feeling tired lately' },
    { role: 'bot', content: 'I understand you\'re feeling tired. This could be due to various factors...' }
  ];
  ...
  const result = await intentClassifier.classifyIntent(followUpMessage, conversationHistory);

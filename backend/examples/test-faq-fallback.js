...
class FAQFallbackTester {
  ...
  async testFAQFallback(message) {
    try {
      ...
      if (isFallbackResponse) {
        console.log('❌ ISSUE: Still getting fallback response instead of AI answer');
        return false;
      } else {
      ...
      console.error('❌ Error testing FAQ fallback:', error.response?.data?.message || error.message);
      return false;
    }
  }
  ...
  async runTests() {
    ...
    if (successCount === totalTests) {
      console.log('🎉 All tests passed! FAQ properly falls back to AI when needed.');
    } else {
  ...
  async testIntentRouting() {
    ...
    for (const test of routingTests) {
      try {
        ...
        console.log(`✅ Match: ${actualIntent === test.expectedIntent ? 'YES' : 'NO'}`);
